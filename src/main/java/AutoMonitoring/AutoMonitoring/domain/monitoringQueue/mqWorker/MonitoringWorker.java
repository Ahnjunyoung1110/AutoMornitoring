...
public class MonitoringWorker {
    ...
    void startMessage(ProgramInfo info){
        ...
        log.info("모니터링을 시작합니다. TraceId: {}, 대상 해상도: {}", info.getResolutionToUrl().keySet());
    }
    ...
    public void startMonitoring(ApplicationReadyEvent event){
        int prefetch = 80;
        int concurrency = 80;
        log.info("애플리케이션 준비 완료, 'working.queue.stage1' 큐에 ㄴ대한 소비자(리스너)를 시작합니다.");
        ...
        receiver.consumeManualAck(RabbitNames.Q_WORK, new ConsumeOptions().qos(prefetch))
                .flatMap(delivery -> {
                    ...
                    if (!monitoringConfigHolder.getMonitoringEnabled().get()) {
                        ...
                        rabbitTemplate.convertAndSend(RabbitNames.EX_DELAY, RabbitNames.RK_DELAY_4S, cmd, msg -> {
